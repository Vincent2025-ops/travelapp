...
import { 
  ...
  Trash2, Utensils, ShoppingBag, BedDouble, Camera, Share, 
  Search, Mic, ArrowDown, X, Edit3, GripVertical, Check, 
  Loader2, ExternalLink, Menu, DollarSign, Sun, CloudRain, Cloud, 
  Copy, Image as ImageIcon, Settings, Info, ArrowLeft,
  LayoutGrid, List as ListIcon, Globe, Download, AlertTriangle, Share2,
  Smartphone, UploadCloud, FileText, Minus
} from 'lucide-react';
...
const fetchPlacesFromGoogleSheet = async () => {
  try {
    ...
    console.warn("無法讀取 Google Sheet 資料", error);
    return [];
  }
};
...
const CATEGORIES = {
  ...
  fun: { label: '遊樂', icon: Settings, color: 'bg-purple-100 text-purple-600 border-purple-200' },
...
const fallbackCopyText = (text, onSuccess, onError) => {
  ...
  try { 
    ...
    successful ? onSuccess() : onError(new Error("Copy command failed"));
  } catch (err) {
    onError(err);
  }
...
const ToastContainer = ({ toasts }) => (
  <div className="fixed top-4 left-0 right-0 z-[120] flex flex-col items-center pointer-events-none space-y-2">
    {toasts.map((toast) => (
      <div 
        key={toast.id} 
...
const PermissionModal = ({ type, onConfirm, onCancel }) => {
    ...
    const { title, desc, icon: Icon } = config[type] || config.location;

    return (
        <div className="fixed inset-0 bg-black/60 z-[130] flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in">
...
const ToolsModal = ({ onClose, onExport, onImport, allTrips, activeTripId, onInstall }) => {
    ...
    // Alias prop to match the variable name used in the snippet
    const onInstallApp = onInstall;
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in">
             <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <div className="mb-6">
                    <label className="text-xs font-bold text-gray-500 mb-1 block uppercase">選擇操作對象</label>
                    <select 
                        value={selectedTripToShare} 
                        onChange={(e) => setSelectedTripToShare(e.target.value)}
                        className="w-full p-3 border-2 border-yellow-200 rounded-xl text-gray-700 font-bold bg-yellow-50 focus:border-yellow-400 outline-none"
                    >
          ...
          {allTrips.map(trip => (
             ...
             >
                ...
                <button 
                    ...
                    className="absolute top-4 right-4 p-3 bg-white rounded-full text-gray-300 hover:text-red-500 hover:bg-red-50 shadow-sm z-20 transition-colors"
                >
                    <Trash2 size={18} />
                </button>
             </div>
                                 ...
                                 if (navigator.geolocation) {
                                    navigator.geolocation.getCurrentPosition(
                                        (position) => {
                                            setUserLocation({ lat: position.coords.latitude, lng: position.coords.longitude });
                                            setLocationError(false);
                                            addToast("定位成功！", "success");
                                        },
                                        (err) => {
                                            console.error(err);
                                            setLocationError(true);
                                            addToast("無法取得位置", "info");
                                        }
                                    );
                                 }
                            }); 
...
export default function App() {
  ...
  useEffect(() => { localStorage.setItem('activeTab_v1', activeTab); }, [activeTab]);
  useEffect(() => { localStorage.setItem('currentDay_v1', currentDay); }, [currentDay]);
    ...
    if (activeTripId && allTrips.length > 0) {
      ...
      if (trip) {
        ...
        if (!trip.days[currentDay]) {
            ...
            setCurrentDay(dayKeys.length > 0 ? (dayKeys.includes(currentDay) ? currentDay : dayKeys[0]) : 'Day 1');
        }
      } else {
  ...
  useEffect(() => { if (!activeTripId) setFocusedMapItem(null); }, [activeTripId]);
  ...
  const handleDurationChange = (newDuration) => {
    ...
    newDates.forEach(day => {
        if (!currentDaysData[day]) currentDaysData[day] = [];
        if (!currentNotesData[day]) currentNotesData[day] = "";
    });

    const updated = { ...itinerary, dates: newDates, days: currentDaysData, notes: currentNotesData };
    updateItinerary(updated);
  ...
  const handleCopyItinerary = () => {
    ...
    if (navigator.share) {
        navigator.share({ title: itinerary.destination, text: textToCopy }).catch((error) => console.log('Error sharing', error));
    } else {
            ...
            {items.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.location && (
                      <button 
                        onClick={(e) => { e.stopPropagation(); setFocusedMapItem(item); setActiveTab('map'); }} 
